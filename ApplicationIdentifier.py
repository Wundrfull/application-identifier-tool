...
class Application(tk.Tk):
    ...
    def create_or_update_overlay(self, title):
        ...
        # Schedule updates
        self.schedule_overlay_update(title, canvas)
    ...
    def update_overlay_canvas(self, canvas, target_window):
        ...
        text_position = (target_window.width - 10, target_window.height - 20)
    ...
    def schedule_overlay_update(self, title, canvas):
        # Check if the window and canvas still exist before updating
        if title in gw.getAllTitles():
            self.update_overlay_canvas(canvas, gw.getWindowsWithTitle(title)[0])
            # Reschedule the update
            self.update_task_id = self.after(1000, lambda: self.schedule_overlay_update(title, canvas))
        else:
